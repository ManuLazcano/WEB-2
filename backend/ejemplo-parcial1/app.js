...
/**
 * Asigna al proyecto con id=_IDPROYECTO_ al programador con id=_IDPROGRAMADOR_. 
 * No hay datos de entrada (enviar {}). Salida: "ok". No es necesario revisar si la asignación ya existe.
 */
app.post('/proyectos/:idproyecto/programadores/:idprogramador', async (req, res) => {
  const { idproyecto, idprogramador } = req.params;

  try {
    const unProyecto = await Proyecto.findByPk(idproyecto);
    if (unProyecto === null) {
      res.status(404).json({ error: `No se encontró el proyecto con ID ${idproyecto}.` });
    } else {
      const unProgramador = await Programador.findByPk(idprogramador);
      if(unProgramador === null) {
        res.status(404).json({ error: `No se encontró el programador con ID ${idprogramador}.` });
      } else {
        await unProyecto.addProgramadores(unProgramador);
        res.json({message: "ok"});
      }         
    }
  } catch (error) {
...
/**
 * Desasigna al proyecto con id=_IDPROYECTO_ al programador con id=_IDPROGRAMADOR_. 
 * No hay datos de entrada (enviar {}). Salida: "ok". No es necesario revisar si la asignación ya existe.
 */
  ...
  try {
    ...
    if (unProyecto === null) {
      ...
      if(unProgramador === null) {
        ...
        await unProyecto.removeProgramadores(unProgramador);
        res.json({message: "ok"});
      }         
    }
  } catch (error) {
